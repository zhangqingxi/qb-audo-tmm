...
# 获取最小的的一个文件
def get_least_content(data=None, index=None, limit_split_torrent_download_size=None):
    item = None
    for row in data:
        ...
        if 100 * 1024 >= row['size'] or row['size'] >= Tool(number=int(limit_split_torrent_download_size[1])).to_byte(
            unit='GB').value:
            continue
...
# 返回可下载的文件序号
def get_download_content_index(content=None, least_size=0, content_index=None, limit_split_torrent_download_size=None):
    if content_index is None:
        content_index = []
    least_content = get_least_content(data=content, index=content_index,
    ...
    if least_content is not None:
        ...
        if Tool(number=int(limit_split_torrent_download_size[0])).to_byte(unit='GB').value > least_size:
        ...
        # 剩余空间不足与下载
        if Qb.free_space - least_size <= Tool(number=Qb.less_disk_space).to_byte('GB').value:
            return ''
...
def log_content(item=None):
    info = {}
    if item['state'] in Qb.active_torrent_state:
        info = {
            'state': item['state'],
    ...
    if data is None:
        data = {
            ...
            'completion_time': time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(item['completion_on'])) if item[
    ...
    def torrent_content(self, item=None):
        ...
        if self.response['code'] == 200:
            ...
            # 文件可拆分
                ...
                limit_split_torrent_download_size = str(
                    os.getenv(self.qb_name + '_LIMIT_SPLIT_TORRENT_DOWNLOAD_SIZE')).split(',')
                download_index = get_download_content_index(content=content,
                                                            limit_split_torrent_download_size=limit_split_torrent_download_size)
                if download_index != '':
                    no_download_index = []
                    for row in content:
